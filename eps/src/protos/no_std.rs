    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(42) => msg.resp = protos::no_std::mod_EpsResponse::OneOfresp::batteryState(r.read_message::<protos::no_std::BatteryState>(bytes)?),
                Ok(t) => { r.read_unknown(bytes, t)?; }
                Err(e) => return Err(e),
            }
        }
...
impl MessageWrite for EpsResponse {
    fn get_size(&self) -> usize {
        ...
        + match self.resp {
            ...
            protos::no_std::mod_EpsResponse::OneOfresp::batteryState(ref m) => 1 + sizeof_len((m).get_size()),
    ...
    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        match self.resp {            protos::no_std::mod_EpsResponse::OneOfresp::railState(ref m) => { w.write_with_tag(18, |w| w.write_message(m))? },
            ...
            protos::no_std::mod_EpsResponse::OneOfresp::batteryState(ref m) => { w.write_with_tag(42, |w| w.write_message(m))? },
...
pub enum OneOfresp {
    ...
    batteryState(protos::no_std::BatteryState),
    None,
}
